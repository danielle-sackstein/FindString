...
#define MAX_BYTES_IN_LINE            (MAX_CHARS_IN_LINE + 2 + 1)
...
void printLine(char *printFileName, char *line, size_t lineLength)
{
	...
	char lastChar = line[lineLength-1];
...
/**
 * Checks if the letters are equal (they are equal even if they have different case).
 ...
 * @param wordLength the number of characters in the wordToFind
 * @param line a pointer to the line that is checked.
 * @param lineLength the number of characters in the line (including '\r', '\n' if any)
 * @return if the word was found in the line, false otherwise
 */
bool findWordsInLine(char *wordToFind, size_t wordLength, char *line, size_t lineLength)
{
...
int findWordsInFile(char *wordToFind, char *fileName, char *printFileName)
{
	...
	while (true)
	{
		...
		bool found = findWordsInLine(wordToFind, wordLength, line, lineLength);
		if (found)
		{
			printLine(printFileName, line, lineLength);
		}
	}
